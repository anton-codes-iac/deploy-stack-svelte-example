...
resource "aws_cloudwatch_log_group" "app_logs" {
  name              = "/ecs/deploy-stack-svelte-example"
...
resource "aws_iam_role" "execution_role" {
  name               = "deploy-stack-svelte-example-execution-role"
...
resource "aws_iam_role" "task_role" {
  name               = "deploy-stack-svelte-example-task-role"
  assume_role_policy = data.aws_iam_policy_document.ecs_trust.json
}

# --- ECS Cluster ---
resource "aws_ecs_cluster" "main" {
  name = "deploy-stack-svelte-example-cluster"
}

# --- ECS Task Definition ---
resource "aws_ecs_task_definition" "app" {
  family                   = "deploy-stack-svelte-example-task"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name      = "deploy-stack-svelte-example-container"
...
resource "aws_lb" "main" {
  name               = "deploy-stack-svelte-example-alb"
...
resource "aws_lb_target_group" "app" {
  name        = "deploy-stack-svelte-example-tg"
  port        = 8080
  protocol    = "HTTP"
...
resource "aws_ecs_service" "app" {
  name            = "deploy-stack-svelte-example-service"
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.app.arn
    container_name   = "deploy-stack-svelte-example-container"
    container_port   = 8080
  }
